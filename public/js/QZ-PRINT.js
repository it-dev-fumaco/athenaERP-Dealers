...
function deployQZApplet() {
    ...
    var attributes = {id: "qz", code:'qz.PrintApplet.class',
        archive:'file:///c:/Program%20Files/QZ%20Tray/qz-print.jar', width:1, height:1};
...
function set_default(){
	document.getElementById("x_distance").value = Number(120);
	document.getElementById("y_distance").value = Number(80);
	document.getElementById("column").value = Number(4);
	document.getElementById("rows").value = Number(2);
...
function findPrinter(name) {
	...
	var part1 = barcode.slice(0,7)
	var part2 = barcode.slice(7)
	
	var quantity = Number(document.getElementById("quantity").value);
	var column_y_1 = Number(document.getElementById("column").value);
	var rows_x_1 = Number(document.getElementById("rows").value);
	var to_print=0;
	var Area = column_y_1*rows_x_1;
	var distance_row = Number(document.getElementById("x_distance").value);
	var distance_column = Number(document.getElementById("y_distance").value);
    ...
    if (isLoaded()) {
        ...
        window['qzDoneFinding'] = function() {
            ...
            p.value + '" not found.');
		if(quantity>Area){
			qz.append('^XA~TA000~JSN^LT0^MMT^MNW^MTT^PON^PMN^LH0,0^JMA^PR5,5^MD10^JUS^LRN^CI0^XZ^XA^LL0160^PW480^CF0,20\n');
			while(quantity>Area){
			quantity = quantity - Area;
			to_print = to_print+1;
			var coordinate_y = 76;
			for(barcode_rows=0;barcode_rows<rows_x_1;barcode_rows++){
				coordinate_y=coordinate_y + (barcode_rows*distance_column);
				var coordinate_x=22;
				for(barcode_column=0;barcode_column<column_y_1;barcode_column++){
					coordinate_x1 = coordinate_x + (barcode_column*distance_row);
					
					//var new_code = '^BY72,72^FT' + coordinate_x1 + ',' + coordinate_y + '^BXN,4,200,0,0,1^FH\^FD'+barcode;
					qz.append('^BY72,72^FT' + coordinate_x1 + ','+coordinate_y+'^BXN,4,200,0,0,1^FH^FD'+barcode+'^FS');
					qz.append('^FT'+coordinate_x1+','+(coordinate_y+20)+'^FD'+part1+'^FS');
					qz.append('^FT'+coordinate_x1+','+(coordinate_y+40)+'^FD'+part2+'^FS');
						}
					}	
					
			}
			//qz.append('^PQ'+ to_print +',0,1,Y\n');
			qz.append('^PQ'+to_print+',0,1,Y^XZ');
			
			if(quantity!=Area){
			qz.append('^XA~TA000~JSN^LT0^MMT^MNW^MTT^PON^PMN^LH0,0^JMA^PR5,5^MD10^JUS^LRN^CI0^XZ^XA^LL0160^PW480^CF0,20\n');
			coordinate_y1=76;
			var y_coordinate=0
				while(quantity>column){
				quantity=quantity-column;
				y_coordinate=y_coordinate+1;
				coordinate_y1=coordinate_y1+(y_coordinate*distance_column);
				
				coordinate_x = 22;
				for(barcode_column=0;barcode_column<column;barcode_column++){
					coordinate_x2 = coordinate_x +(barcode_column*distance_row);
					qz.append('^BY72,72^FT' + coordinate_x2 + ',' + coordinate_y1 + '^BXN,4,200,0,0,1^FH\^FD'+barcode+'^FS');
					qz.append('^FT'+coordinate_x2+','+(coordinate_y1+20)+'^FD'+part1+'^FS');
					qz.append('^FT'+coordinate_x2+','+(coordinate_y1+40)+'^FD'+part2+'^FS');
				}
				}
				
			
				
				var coordinate_x3=22;
				while(quantity!=0){
					quantity=quantity-1;
					coordinate_x3 = coordinate_x3 + Number(distance_row);
					x_coordinate = coordinate_x3 - Number(distance_row);
					qz.append('^BY72,72^FT' + x_coordinate + ',76^BXN,4,200,0,0,1^FH\^FD'+barcode+'^FS');
					qz.append('^FT'+x_coordinate+',100^FD'+part1+'^FS');
					qz.append('^FT'+x_coordinate+',120^FD'+part2+'^FS');
					
					
				}
			qz.append('^PQ1,0,1,Y^XZ')
			}
		}
		else{
			qz.append('^XA~TA000~JSN^LT0^MMT^MNW^MTT^PON^PMN^LH0,0^JMA^PR5,5^MD10^JUS^LRN^CI0^XZ^XA^LL0160^PW480^CF0,20\n');
			...
			qz.print();
			//new_code = barcode +'^FS^BY72,72^FT142,76^BXN,4,200,0,0,1^FH\^FD'+ barcode +'^FS^BY72,72^FT262,76^BXN,4,200,0,0,1^FH\^FD'+ barcode +'^FS^BY72,72^FT381,76^BXN,4,200,0,0,1^FH\^FD'+ barcode +'^FS^BY72,72^FT22,156^BXN,4,200,0,0,1^FH\^FD'+ barcode +'^FS^BY72,72^FT142,156^BXN,4,200,0,0,1^FH\^FD'+ barcode +'^FS^BY72,72^FT262,156^BXN,4,200,0,0,1^FH\^FD'+ barcode +'^FS^BY72,72^FT381,156^BXN,4,200,0,0,1^FH\^FD'+ barcode +'^FS^PQ'+ barcode_quantity +',0,1,Y';
			
			/*qz.append('^XA~TA000~JSN^LT0^MMT^MNW^MTT^PON^PMN^LH0,0^JMA^PR5,5^MD10^JUS^LRN^CI0^XZ^XA^LL0160^PW480^BY72,72^FT22,76^BXN,4,200,0,0,1^FH\^FD\n')
			qz.append(new_code+'^XZ\n');
			qz.append(new_code+'^XZ\n');
			qz.print();*/
            // Remove reference to this function
            window['qzDoneFinding'] = null;
...
function printZPL() {
    ...
    // Hint:  Carriage Return = \r, New Line = \n, Escape Double Quotes= \"
    qz.append('^XA~TA000~JSN^LT0^MMT^MNW^MTT^PON^PMN^LH0,0^JMA^PR5,5^MD50^JUS^LRN^CI0^XZ^XA^LL0203^PW559^FO60,7,0^BQN,2,3^FD ^FS^PQ1,0,1,Y^XZ\n');
	qz.print();
	
}
function printersas() {
  qz.append("^XA~TA000~JSN^LT0^MMT^MNW^MTT^PON^PMN^LH0,0^JMA^PR5,5^MD50^JUS^LRN^CI0^XZ^XA^LL0203^PW559\n");
  qz.print();
}
